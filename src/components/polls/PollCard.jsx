...
export default function PollCard({ poll, user, userVote, onVoteSubmit, onViewDetails, onDelete, isLocked, userPledge }) {
  ...
  useEffect(() => {
    const checkUserAccess = async () => {
      // If poll is not premium, or user is not logged in, or user is admin, they have access
      ...
      // If poll is premium and user is not admin, check for active subscription
      try {
  ...
  if (!isAdmin && !isCheckingAccess && isLocked) { // Now uses the isLocked prop
    return (
