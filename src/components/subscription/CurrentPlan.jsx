...
export default function CurrentPlan({ subscription, onUpgrade, onCancelSubscription, isCancelling }) {
  ...
  const [subscriptionRefundsEnabled, setSubscriptionRefundsEnabled] = useState(true);
  ...
  useEffect(() => {
    const loadSettings = async () => {
      try {
        const settings = await PlatformSetting.filter({ setting_key: 'subscription_refund_enabled' });
        if (settings.length > 0) {
          const value = settings[0].setting_value;
          setSubscriptionRefundsEnabled(value === 'true' || value === true);
        }
      } catch (error) {
  ...
  // Show Cancel button only if subscription is active and NOT already cancelling
  const showCancelButton = subscription.status === 'active' && !subscription.cancelAtPeriodEnd && subscription.plan_type !== 'basic';
  
  // Show Reactivate button if subscription is set to cancel at period end
  const showReactivateButton = subscription.status === 'active' && subscription.cancelAtPeriodEnd;
  
  const showRefundButton = !isLoadingSettings && subscriptionRefundsEnabled && subscription.status === 'active' && subscription.plan_type !== 'basic';
    ...
    <>
      <Card className="relative overflow-hidden border-0 shadow-xl">
        ...
        <CardContent className="relative">
          ...
          {/* Plan Details Grid */}
          <div className="grid grid-cols-1 md:// ... keep existing code (Cancel, Reactivate, Refund buttons) ...-cols-2 gap-4 mb-6">
            {/* Price Card */}
          ...
          <div className="space-y-3">
            {/* Upgrade Button - Light Gradient */}
            {subscription.plan_type !== 'vip' && subscription.status === 'active' && !subscription.cancelAtPeriodEnd && (
              <Button 
                onClick={onUpgrade} 
                className="w-full h-12 bg-gradient-to-r from-purple-400 via-pink-400 to-rose-400 hover:from-purple-500 hover:via-pink-500 hover:to-rose-500 text-white font-semibold shadow-lg hover:shadow-xl transition-all duration-300"
              >
            ...
            {/* Reactivate Button - Show if subscription is being cancelled */}
            ...
            {showRefundButton && !subscription.cancelAtPeriodEnd && (
