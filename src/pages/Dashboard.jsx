...
export default function Dashboard() {
  ...
  const loadDashboardData = useCallback(async (currentUser, isMounted, abortController) => {
    ...
    try {
      ...
      else setUser(null); // Explicitly set user to null for guests
      ...
      if (currentUser && !abortController.signal.aborted && isMounted) { // Only fetch for authenticated users
        setTimeout(async () => {
          if (!isMounted || abortController.signal.aborted) return;
      ...
      } else if (!currentUser && isMounted && !abortController.signal.aborted) {
        // If no current user, ensure subscription and referrals are cleared/defaulted
  ...
  useEffect(() => {
    ...
    const checkUserRoleAndRedirect = async () => {
        try {
            ...
            // For traders, admins, super_admins, AND GUESTS - load the dashboard
            ...
            if (isMounted && !abortController.signal.aborted && !error.message?.includes('aborted')) {
                console.log("Loading dashboard in guest mode");
                // Load dashboard for guests even on error
                loadDashboardData(null, isMounted, abortController);
            }
        }
    };
