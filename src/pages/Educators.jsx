...
export default function Educators() {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        // Initialize courses and workshops with sample data.
        // As there's no real fetching mechanism for them defined in this outline,
        // we'll ensure they are always set.
        setCourses(sampleCourses);
        setWorkshops(sampleCourses.filter(c => c.course_type === 'live_workshop'));

        // Load educators with error handling
        const loadedEducators = await Educator.filter({
          status: 'approved'
        }, '-student_count', 50).catch(() => []); // Added limit 50

        if (!isMounted || abortController.signal.aborted) return;

        if (loadedEducators.length > 0) {
          setEducators(loadedEducators);
        } else {
          // Use sample data if no educators loaded from API
          setEducators(sampleEducators);
        }

        // Only check subscription if a user is logged in
        if (currentUser) {
          if (!['admin', 'super_admin'].includes(currentUser.app_role)) { // For regular users, check for active subscription
            try {
          ...
          } else { // Admin/Super Admin always have premium access
            setHasSubscription(true);
          }
        } else { // No currentUser means no subscription
          setHasSubscription(false);
        }
        ...
        console.log("Loading educators, courses, and workshops in guest mode with sample data due to an error.");
        setEducators(sampleEducators);
        setCourses(sampleCourses); // Ensure courses are set even on error
        setWorkshops(sampleCourses.filter(c => c.course_type === 'live_workshop')); // Ensure workshops are set even on error
        setUser(null);
        setHasSubscription(false);
      } finally {
        if (isMounted && !abortController.signal.aborted) {
  ...
  }, []); // Empty dependency array as loadData is defined internally and doesn't depend on external state/props
  ...
  const canAccessPremium = hasSubscription; // Updated to use the new hasSubscription state
