...
export default function Finfluencers() {
  ...
  const [hasSubscription, setHasSubscription] = useState(false); // Changed from `subscription` to `hasSubscription`
  ...
  // loadData is wrapped in useCallback and passed abortSignal to handle component unmounts
  const loadData = useCallback(async (abortSignal) => {
    ...
    try {
      ...
      if (loadedInfluencers.length > 0) {
        ...
        // Use sample data if no influencers loaded or API call failed
        setInfluencers(sampleInfluencers);
      }

      // Initialize videos and courses with sample data, as fetching them from API might not be critical or stable
      ...
      // Try to enhance with real data for videos if possible (but don't block on it)
      // This section was in the original code, adapted to new structure.
      try {
        const [fetchedVideos] = await Promise.all([
          InfluencerPost.filter({ status: 'approved' }, '-view_count', null, abortSignal).catch(() => []) // Pass abortSignal
        ]);
      ...
      // Fallback to sample data and guest mode if any critical error occurs
      console.log("Loading finfluencers in guest mode with sample data due to an error.");
      setInfluencers(sampleInfluencers);
      setVideos(sampleVideos);
      setCourses(sampleCourses);
      setUser(null);
  ...
  }, []); // Empty dependency array because `isMountedRef` and `abortSignal` are handled internally or passed as arguments.

  useEffect(() => {
    isMountedRef.current = true;
    const abortController = new AbortController(); // New AbortController for this effect run

    // Call loadData, passing the signal from the AbortController
    loadData(abortController.signal);

    return () => {
      isMountedRef.current = false;
      abortController.abort(); // Abort any ongoing fetches when component unmounts or effect re-runs
    };
  ...
  // `canAccessPremium` now directly uses `hasSubscription` boolean state
