...
// Sample data for guest users or when no polls are loaded
const samplePolls = [
  {
    id: "sample-1",
    ...
    target_price: 1000,
    target_date: new Date(Date.now() + 3 * 24 * 60 * 60 * 1000).toISOString(), // Example future date
    yes_votes: 120,
    no_votes: 80,
    total_votes: 200,
    is_premium: false,
  ...
  {
    id: "sample-4",
    title: "Google (GOOGL) will outperform S&P 500 next month?",
    stock_symbol: "GOOGL",
    poll_type: "price_target",
    target_price: null, // For general performance comparison
    target_date: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString(), // Example future date
    yes_votes: 90,
    no_votes: 60,
    total_votes: 150,
    is_premium: false,
    ...
    is_active: true,
    description: "Do you believe Google will surpass the S&P 500 index in returns over the next 30 days?",
  },
];
...
export default function Polls() {
  ...
  const [hasSubscription, setHasSubscription] = useState(false); // NEW state
  ...
  const loadData = useCallback(async () => {
    ...
    setIsLoading(true);
    let currentUser = null; // Declare here to be available in finally block if User.me fails

    try {
      // Try to get user, but allow null for guests
      currentUser = await User.me().catch(() => null);
      ...
      let pollsToDisplay = loadedPolls;
      if (loadedPolls.length === 0) {
        // Use sample data if no polls loaded from API
        console.log("No polls loaded from API, using sample data.");
        pollsToDisplay = samplePolls;
      }
      setPolls(pollsToDisplay);
      ...
      if (currentUser && !['admin', 'super_admin'].includes(currentUser.app_role)) {
        try {
          ...
          console.log("No active subscription found for user:", error);
          setHasSubscription(false);
        }
      } else if (currentUser && ['admin', 'super_admin'].includes(currentUser.app_role)) {
        // Admin/Super Admin implicitly have full access
      ...
      console.log("Loading polls in guest mode with sample data due to general error.");
      setPolls(samplePolls); // Fallback to sample polls on any load error
      setUser(null);
      setUserVotes({});
      setHasSubscription(false);
      toast.error("Failed to load polls data. Displaying sample polls.");
    } finally {
      if (isMountedRef.current) {
        setIsLoading(false);
      }
    }
  }, [refreshTrigger]); // refreshTrigger is a dependency to re-run loadData when needed
  ...
  }, [loadData, refreshTrigger]); // loadData is already a useCallback and its dependencies (like refreshTrigger) are handled there.
  ...
  const handleVote = async (pollId, vote) => {
    ...
    try {
      ...
      } else { // pledge_poll or buy_sell_hold
      ...
      // Instead of listing again, trigger refresh
      ...
      loadData(); // Re-load data in case of error
    }
  };

  const handleCreatePoll = async (pollData) => {
    // Require login for creating polls
    if (!user) {
      toast.error("Please log in to create a poll!");
      setShowCreateModal(false); // Close modal if user is not logged in
      return;
    }
    try {
