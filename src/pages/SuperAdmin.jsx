...
export default function SuperAdmin() {
  ...
  const renderContent = () => {
    ...
    const activeEntityConfig = Array.isArray(entityConfigs) ? entityConfigs.find(e => e.entity_name === activeTab) : null;
  ...
  // If user is null AFTER loading, it means even the mock user failed or wasn't set correctly.
  // This might happen if there's a more fundamental issue, or if the mock user logic itself has issues.
  // In a production environment, this block would handle unauthorized access after isLoading is false.
  // For testing, we generally expect user to be populated with either a real or mock user.
  if (!user) {
    return (
      <div className="flex h-screen items-center justify-center">
        <p>Fatal error: User data not available. Please check console.</p>
      </div>
    );
  }
